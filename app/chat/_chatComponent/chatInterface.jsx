...
async function fetchChat(session, profile, backlog){
    if (!backlog){
        backlog = [
            ...
            { role: "system", content: "Be patient, unthreatening, helpful. Keep the conversation on track. If the user is not talking about travel, steer the conversation back to travel. Ask for more information from the user so that you can build the profile faster. No need to respond with your name. Speak English, unless specfically requested by the user. Start by asking the user where they would like to visit first." },
        ];
        if(profile && Array.isArray(profile)){backlog.push({role: "system", content:`User already has a profile, which is: ${profile}`})};
